...
export default function Liberation({session, liberation, allocation, liberations, enseignantId, onRemove, scenario = "production"}: any){
    ...
    const menuContent = isVisible && (
        ...
        >
            ...
            <p className="mb-2"><button className="btn btn-outline-light btn-sm w-100" onClick={() => { setModalOpen(true); closeMenu(); }}>Changer la quantité</button></p>
            <p className="mb-0"><button className="btn btn-outline-light btn-sm w-100" onClick={() => window.open("/admin/allocations/" + session + "?highlight=" + allocation.id, "_blank")}>Modifier l'allocation</button></p>
        </div>
    )
