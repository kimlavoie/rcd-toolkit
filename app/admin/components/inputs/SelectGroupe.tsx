...
export default function SelectGroupe({value, onChange, session}: {value: string, onChange: (v: string) => void, session?: string}){
            ...
            {sortedGroupes.map((groupe) => {
                ...
                return (
                    <option key={groupe.id} value={groupe.id}>
                        {cours?.sigle} - {cours?.nom} ({groupe.nbEtudiants})
                    </option>
                )
            })}
        </select>
